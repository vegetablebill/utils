package com.caijun.utils.url;

import static com.caijun.utils.str.CharUtil.isAlpha;
import static com.caijun.utils.str.CharUtil.isDigit;
import static com.caijun.utils.str.CharUtil.isPchar;
import static com.caijun.utils.str.CharUtil.isSubDelimiter;
import static com.caijun.utils.str.CharUtil.isUnreserved;
 ...
 * {@literal https://jodd:ddoj@www.jodd.org:8080/file;p=1?q=2#third}. It consist
 * of:
 * <ul>
 * <li>scheme (https)</li>
 * <li>user (jodd)</li>
...
public class URLCoder {
	private static String ENCODING = StringUtil.ENCODING;
	...
	enum URIPart {
		 ...
		 * component.
		 * 
		 * @return <code>true</code> if the character is allowed; {@code false}
		 *         otherwise
		 */
	 ...
	 */
	private static String encodeUriComponent(String source, String encoding,
			URIPart uriPart) {
		if (source == null) {
			return null;
		}
	 ...
	 * public static String encodeAuthority(String authority) { return
	 * encodeUriComponent(authority, ENCODING, URIPart.AUTHORITY); }
	 */
	 ...
	 * sets.
	 * <p>
	 * This method does <b>not</b> attempt to encode "=" and "{@literal &}"
	 * characters in query parameter names and query parameter values because
	 * they cannot be parsed in a reliable way.
	 */
	public static String encodeUri(String uri, String encoding) {
		...
		if (m.matches()) {
			...
			return encodeUriComponents(scheme, authority, userinfo, host, port,
					path, query, fragment, encoding);
		}
	...
	public static String encodeHttpUrl(String httpUrl) {
		return encodeHttpUrl(httpUrl, ENCODING);
	}

	/**
	 * Encodes the given HTTP URI into an encoded String. All various URI
	 * components are encoded according to their respective valid character
	 * sets.
	 * <p>
	 * This method does <b>not</b> support fragments ({@code #}), as these are
	 * not supposed to be sent to the server, but retained by the client.
	 * <p>
	 * This method does <b>not</b> attempt to encode "=" and "{@literal &}"
	 * characters in query parameter names and query parameter values because
	 * they cannot be parsed in a reliable way.
	...
	public static String encodeHttpUrl(String httpUrl, String encoding) {
		...
		if (m.matches()) {
			...
			return encodeUriComponents(scheme, authority, userinfo, host,
					portString, path, query, null, encoding);
		}
		throw new IllegalArgumentException("Invalid HTTP URL: " + httpUrl);
	}

	private static String encodeUriComponents(String scheme, String authority,
	...
	/**
	 * Creates URL builder for user-friendly way of building URLs. Provided path
	 * is parsed and {@link #encodeUri(String) encoded}.
	 * 
	...
	/**
	 * Creates URL builder with given path that can be optionally encoded. Since
	 * most of the time path is valid and does not require to be encoded, use
	 * this method to gain some performance. When encoding flag is turned off,
	 * provided path is used without processing.
	 * <p>
	 * The purpose of builder is to help with query parameters. All other URI
	 * parts should be set previously or after the URL is built.
	 */
	public static Builder build(String path, boolean encodePath) {
		return new Builder(path, encodePath, ENCODING);
	}
	...
	public static class Builder {
		...
		public Builder(String path, boolean encodePath, String encoding) {
			...
			this.hasParams = url.indexOf(StringUtil.QUESTION_MARK) != -1;
		}
